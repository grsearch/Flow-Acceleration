...
const { SmartPullbackShadowManager } = require('./SmartPullbackShadowManager');

class SmartPullbackShadowSuite {
  constructor({ config, store, now = () => Date.now() }) {
    this.config = config;
    this.managers = new Map();
    for (const cohort of config.cohorts || []) {
      this.managers.set(cohort.id, new SmartPullbackShadowManager({
        config: {
          ...config,
          ...cohort,
          cohortId: cohort.id,
  ...
  onSmartWalletBuy(event) {
    ...
    for (const manager of this.managers.values()) {
      const result = manager.onSmartWalletBuy(event);
...
module.exports = { SmartPullbackShadowSuite };
