...
async function main() {
  ...
  assert.ok(dashboard.includes('P_A/P_B只按成熟的毕业前首次OPEN评价'));
  assert.ok(dashboard.includes('60天长期精英不能替代毕业预测票权'));
  assert.ok(dashboard.includes('近24h亏损不禁用其交易/持仓研究资格'));
  assert.ok(dashboard.includes('未平仓显示 OPEN / PARTIAL，不是 NO_EXIT'));
  assert.ok(dashboard.includes('首轮全量回填'));
  ...
  assert.ok(dashboard.includes('来自严格的3个P_A毕业预测独立集群'));
