...
async function main() {
  ...
  assert.ok(dashboard.includes('长期精英组只要求完整60天真实已实现收益严格超过200 SOL'));
  assert.ok(dashboard.includes('近24h即使亏损也不禁票'));
  assert.ok(dashboard.includes('未平仓显示 OPEN / PARTIAL，不是 NO_EXIT'));
  assert.ok(dashboard.includes('首轮对当前 Registry 全量回填'));
  ...
  assert.ok(dashboard.includes('长期精英仍须通过风险和独立集群检查'));
