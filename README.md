...
- 五组均在各自快照后200ms的首笔 Bonding Curve 成交模拟入场；旧30秒组和旧 `XT15_H120` cohort 保持原ID，不重算历史。

每个入场与十种独立卖出规则交叉建仓：固定5秒/15秒、旧 `XT15_H120`、`+20%/回撤10%/180秒`、`+30%/回撤15%/300秒`、`+30%减仓50%+尾仓回撤20%`、60秒新增买家速度及增量净流入转弱退出，以及三组阶梯移动止盈。
...
该策略只写入独立表 `holder_growth_shadow_positions`，默认按1 SOL仓位扣除确定性成本；50%减仓组额外扣除一次链上执行的固定成本。不会修改现有实盘策略，也不会签名或发送交易。每日研究导出会包含该表。
