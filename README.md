...
## FEA-OBS 信号特征有效性审计（2026-08-27）

`FEA-OBS` 不再继续堆叠固定阈值策略，而是把已有公开链上信号拆成五类独立
特征：资金流、参与者扩散、买卖平衡、价格结构与可执行容量。每个符合最低
公共样本门槛的信号只记录一次前向观察，并分别标注 5/30/120/300 秒的价格
收益、1 SOL 储备容量可执行收益、MFE 与 MAE；缺少后续成交的样本保持右删失，
不会按亏损或盈利填充。

Dashboard 会同时展示单一特征、特征分数和无额外特征基线的样本量、胜率、
平均/中位收益、PF、Big50/Big100 与 RUG 比例，用来判断当前指标是否真的具有
样本外区分能力。该观察器只复用现有成交与信号流，写入独立表
`feature_edge_audit_observations`；不增加 RPC、不建立实盘或 Shadow 仓位、
不读取私钥，也不会签名或发送交易。
...
作为审计对照，单独统计退订后是否出现 `Big50/Big100/Big200`，从而估算大赢家
漏检率。Dashboard 同时展示各层活跃数、退订原因、5/30/60 分钟收益、MFE、
MAE 和 1 SOL 可执行回收率。实际 gRPC 订阅仍是所有模块请求 Mint 的并集；
如果其他策略仍在跟踪同一 Mint，本观察器退订不会强制终止其他策略的数据。

该模块只保存迁移、阶段门槛和里程碑快照，不保存长期逐笔流水；每个 Mint 的
内存事件有固定上限。它不建立实盘或 Shadow 仓位、不增加 RPC、不读取私钥，
