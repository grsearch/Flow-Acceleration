...
滚动 Smart Wallet Registry 的投票资格使用钱包自己的真实买卖账本，不再使用“跟随钱包后模拟买入并持有30/300秒”的收益。账本按加权平均成本核算 `ADD / REDUCE`，只有完整 `CLOSE` 的仓位进入已实现收益；普通组要求滚动24小时至少一笔完整平仓、已实现 SOL 收益大于0且资金回报率大于0。长期精英组是独立收益资格：在历史账本完整的前提下，滚动60天真实已实现收益必须严格超过200 SOL；一旦满足，近24小时即使亏损也不会被禁票，并按1个有效票权参与共识。长期精英身份不伪装成 `S_A`，链上 AGE、风险状态和独立集群防关联门槛仍然保留。未平仓只显示 `OPEN / PARTIAL`，不会标成 `NO_EXIT`，也不进入已实现收益。7天和30天真实已实现收益继续用于 `S / C / H` 分级；历史 `smart_wallet_forward_labels` 只保留研究兼容，不再新增，也不再影响资格或分级。
