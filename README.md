...
滚动 Smart Wallet Registry 把“毕业预测”和“交易盈利”拆成两个正交维度。`S_A / S_B / S_C` 现在只衡量钱包在 Bonding Curve 首次 `OPEN` 后12小时内预测毕业的能力：滚动60天、每个钱包/Mint只计一次、最近未满12小时的样本右删失，并永久排除用于发现该钱包的 seed Mint。`S_A` 默认至少30个成熟样本、3个活跃日、毕业率不低于25%、相对 Registry 成熟Mint基线至少1.5倍，且95% Wilson下界不低于10%。`C / H` 仍按钱包自己的真实买卖账本衡量盈利稳定性与大赢单持仓能力；账本按加权平均成本核算 `ADD / REDUCE`，只有完整 `CLOSE` 进入已实现收益。普通收益资格仍要求滚动24小时至少一笔完整平仓且收益与资金回报都大于0；长期精英仍按完整60天真实已实现收益严格超过200 SOL识别，近24小时亏损不取消其交易/持仓研究资格，但长期盈利不再自动产生毕业预测票权。未平仓只显示 `OPEN / PARTIAL`，不会标成 `NO_EXIT`。

主共振从 `GRAD_PREDICTION_V1` 起严格失败关闭：`PA3_POST_FLOW_V1`、`PA3_SCOUT15_FLOW_V1` 与 `PA3_EARLY_C25_V1` 都要求300秒内3个不同的 `S_A` 独立集群，三票权重均为1；池内 `S_A` 不足时不降级、不发信号。原滚动宽松规则保留在 `ROLLING_DYNAMIC_CONTROL_V1` 与 `EARLY_C25_R3`，只用于观察新增钱包导致的假共振稀释，不能通过主 Smart Consensus Overlay 门控。旧 `POST_FLOW`/`SCOUT15_FLOW` 系列停止新增记录，只保留历史，避免规则变更前后的样本混在同一 cohort。`EB_A_SWC_PA3_W300` 同样要求3个 `S_A` 独立集群，原 `EB_A_SWC_R2_W300` 保留为宽松对照。所有新 cohort 都是前向记录，不回填也不混入旧规则历史行。

系统通过 Helius `getTransactionsForAddress` 为 Registry 钱包回填历史成交：第一次部署会为当时已经在 Registry 中的全部钱包建立一次性全量队列，不受每日50个限制；以后滚动发现的新钱包按北京时间每天最多启动50个。分页游标、页数、预计 Credits、错误和完成状态都持久化，重启后从断点继续。默认回填60天并额外读取30天预热区间用于补齐跨窗口持仓成本；只有账本重建没有孤立卖出、且60天覆盖完整的钱包才可能进入长期精英组。

自动发现钱包的独立集群确认观察期为12小时，并要求至少出现3个不同 Mint 的首次 `OPEN`。系统使用同 Mint 首次建仓时间与金额行为识别关联地址：默认要求5秒内同步、买入金额差不超过15%，至少在2个 Mint 上重复，且覆盖样本较少一方至少50%的交易。相关地址会合并为同一集群，只计一张共识票；已确认的关联关系是粘性的，后续滚动窗口暂时缺少同步样本也不会自动拆开。12小时只缩短集群确认等待，不改变链上 AGE 门槛、分级条件和滚动24小时真实盈利门槛。

`Smart Wallet Consensus Overlay V1` 为仍在前向验证或已有实盘候选依据的 G、O 与 `FEA-BNH-120` 增加并行条件组。原 Shadow 表、原 cohort、原入场与退出计算全部保留；Overlay 只读取原信号及其最终模拟结果，并检查同 Mint 在原信号前15分钟内是否已有 `PA3_*_V1` 严格毕业预测共识。宽松对照共识不能通过 Overlay。默认对照 `POST_GE30_R23_F2_ONLY_G2_XLEG`、`POST_GD25_35_X8`、`O_C80_D5_B2_S0_NC:1SOL`、`O90_M5_STAIR120:1SOL` 与 `FEA_BNH_120`，已证伪或退休策略不扩组。该层从部署时刻开始前向记录，不回填旧历史，不接受原信号之后出现的共识，也不会签名、发链或改变任何实盘开仓。Dashboard 的 Smart Wallet 页同时展示原全样本与共识通过子集的通过率、收益、胜率和 PF。
...
毕业持仓共振同样只做前向 Shadow。旧 `POST_GRAD_HOLD3_FLOW2_60` 继续作为“3个独立合格集群仍持仓，再等待60秒公共流确认”的对照；新增 `POST_GRAD_HOLD3_DIRECT` 在首笔经过验证的 Pump AMM 成交确认3个集群仍持仓后，于200ms后的下一笔可执行 AMM 成交模拟入场，不再等待毕业后的 Smart Wallet 新开仓或公共流。每个信号并行记录固定2分钟、固定5分钟，以及“+30%卖80%、余20%回撤20%、最长5分钟”的分批退出。`H_B` 按0.5权重参与，三个独立 `H_B` 集群可达到默认最低权重；该路径不签名、不发链，也不接入任何 Live 策略。
...
另设 `COB_F_LR01_FIX30` 与 `COB_F_LR01_FIX30_RUGX` 严格配对：两组使用完全相同
的 COB-F 公共流信号、0.1 SOL容量、200ms入场延迟、1.5秒时效及固定30秒退出；
基准组只记录RUG标签，只有RUGX组会硬拦截跨Mint毒性钱包、跨Mint毒性模板和极端
协同可砸盘性三个高置信条件。Dashboard 将被拦截交易按空仓0%计入配对组合，独立
展示避开的RUG50/RUG80与误杀赢家，不与此前EB-A时点诊断或P500低频对照混算。
...
### PMO 毕业后公开资金流机会矩阵（2026-09-04）

`PMO` 是新的 Shadow-only 策略。离线单日筛选比较了早期延续、浅回调、
深回调和公开资金流延续四种买点；公开资金流延续组样本相对最多，并保留了
当日唯一超过 `+300%` 的右尾，因此作为前向入场条件：毕业后 10～120 秒，
最近 10/3 秒净流分别不低于 3/0.5 SOL，独立买家不少于 6/2，Top1 买入占比
不高于 50%，峰值回撤不超过 35%。离线归档没有 PumpSwap 专用池储备，所以
该结论只是 mark-price 参数筛选；前向 Shadow 会使用真实储备和 0.1 SOL
容量报价验证。

同一买点并行测试四种退出：

- `H15-A30-D15-X120`：15% 止损，+30% 后按峰值回撤 15% 退出，最长 120 秒；
- `H20-A50-D20-X300`：20% 止损，+50% 后回撤 20%，最长 300 秒；
- `H20-A75-D25-X300`：主右尾候选，20% 止损，+75% 后回撤 25%，最长 300 秒；
- `H25-A100-D30-X600`：更宽右尾，25% 止损，+100% 后回撤 30%，最长 600 秒。

每种退出都生成严格同信号的 `BASE` 与 `RUGX`。BASE 只记录 RUG 标签；
RUGX 只硬拦截 `crossMintToxicWallets`、`crossMintToxicTemplate` 和
`extremeCoordinatedDumpability` 三个高置信灾难特征。被拦截组合按空仓 0%
计入可比组合，Dashboard 直接显示避开的 RUG50/RUG80、误杀赢家和收益提升。
单日回放中这三项没有拦截到该小样本，因此不能宣称过滤已有效；必须依赖前向
严格配对积累结论。PMO 永不进入实盘、签名或发送交易，也不增加 RPC。
